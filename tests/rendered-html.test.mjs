...
test("keeps bilingual content, 3D interaction, and fallback navigation in the product source", async () => {
  ...
  assert.match(generatedScene, /PUBLISHED_SCENE_CONFIG: SceneConfig/);
  assert.match(generatedScene, /Generated by the local Content Studio/);
...
test("keeps the Content Studio write endpoint local to the Vite development server", async () => {
  ...
  assert.match(plugin, /\/__content-studio\/model-cache/);
  assert.match(plugin, /\.content-studio-cache/);
  assert.match(plugin, /generated",\s+"scene-config\.ts"/);
  assert.match(plugin, /persistContentStudioProject/);
