...
任一物件都可点击“拖动摆放”进入独立编辑模式。在房间中直接拖动物体可预览 X/Z 平面位置，数值栏继续负责高度、旋转和缩放；只有点击“确认位置”才会写入草稿。点击“取消摆放”、按 Esc、切换物件、离开场景编辑或关闭工作台都会恢复进入模式前的位置。这个流程同样适用于不创建详情页的纯装饰资产。
...
“场景预览”会把工作台收窄到右侧，未被面板覆盖的房间仍可环顾；普通模式下拖动空白区域会旋转视角，摆放模式下拖动选中物体会移动它。数值调整会实时同步。模型缺失或加载失败时会保留一个轻量占位物，避免场景出现不可见但仍可交互的区域。
