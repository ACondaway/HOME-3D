...
房间从内容配置的 `profile.timezone` 读取 IANA 时区（例如 `Asia/Shanghai`），使用浏览器原生 `Intl` 得到当地日期与时钟。太阳主光在白天沿窗外弧线移动，环境光、雾、窗户色温与地面日照同步变化；晨昏阶段会交叠自然光和室内灯，夜间则由吊灯、研究桌灯和微弱月光维持可探索性。时区在内容工作台修改后会热更新，不会重建 Three.js renderer。
