...
- 两种语言的首屏标题、介绍与引文；
- 城市和时区；
- 12 件内置数字资产的名称、章节标题、特质、摘要、介绍、状态、更新时间和引文；
- 每件资产的数据指标；
- 每件资产的内容卡片；
- 镜子页面的个人照片、替代文本和社交媒体链接；
- 相机页面的照片上传、替代文本与 Spotlight 主图。
- 场景中现有物件的位置偏移、旋转和缩放；
- 内置物件、装饰资产与交互资产的多维度可确认拖动摆放；
- 自定义 GLB 资产的上传、放置、强调色和用途；
- 自定义资产可设为“仅装饰”或“可交互”：装饰物不会进入索引或创建详情页，可交互资产可在工作台中编辑双语详情内容。
- 内置资产与自定义交互资产共用可组合内容卡：纯文字、图文、链接按钮三种类型，并可设置标准、宽版或整行布局。

保存方式：

1. 每次输入都会即时更新当前页面；
2. 草稿自动写入当前浏览器的 `localStorage`；
3. 照片通过拖放或文件选择写入 `public/uploads/profile`、`public/uploads/photography` 或 `public/uploads/cards`，GLB 模型写入 `public/uploads/models`；JSON 只保存公开路径，不保存 base64；
...
- 内容保存请求必须是 JSON，最大 24 MiB；服务端使用紧凑 JSON 写入，避免合法双语内容因格式化膨胀而超过静态资产上限；
- 图片上传只接受 JPEG、PNG、WebP、AVIF，最大 10MB、单边 10,000 像素且不超过 4,000 万像素，并在上传前解码、在服务端通过文件内容识别格式；
- 模型上传只接受自包含且结构有效的 glTF 2.0 GLB，最大 24 MiB；该上限为 [Cloudflare Pages 单文件 25 MiB 限制](https://developers.cloudflare.com/pages/platform/limits/#file-size)预留余量；
- 服务端除核对容器、JSON 与二进制范围外，还会限制节点、网格实例、图元、访问器展开规模和纹理解码像素，防止异常模型耗尽浏览器内存；
- 当前工作台会明确拒绝需要额外解码器的 Draco、Meshopt、Basis/KTX2 与 AVIF 模型；请从 Blender 等工具重新导出为未使用这些压缩扩展、纹理为 PNG/JPEG/WebP 的 GLB；
- 上传文件使用随机文件名写入仓库的 `public/uploads`，原始文件名不参与路径；
- “清除”或移除资产只解除内容引用，不自动删除磁盘文件；提交前可人工检查并移除 `public/uploads` 中不再使用的图片或模型；
...
Content Studio 的“场景布局”提供两类编辑：

1. 选择任一内置物件，调整相对原始位置的 X/Y/Z 偏移、角度和三轴缩放；点击“恢复原始位置”可清除覆盖。
2. 点击“添加新资产”，上传 GLB 后设置绝对位置、旋转、缩放和强调色。

任一物件都可点击“拖动摆放”进入独立编辑会话，并在三种方式之间切换：

- **平面 X / Z**：沿物体父级的地面平面拖动；
- **高度 Y**：上下拖动改变高度；
- **方向旋转**：绕物体父级 Y 轴拖动改变朝向，按住 Shift 可吸附到 15°。

切换方式会保留同一会话中已经完成的其他维度预览；只有点击“确认全部摆放”才会把位置与方向一次性写入草稿。点击“取消全部预览”、按 Esc、切换物件、离开场景编辑或关闭工作台都会恢复进入会话前的完整变换。这个流程同样适用于不创建详情页的纯装饰资产，数值栏仍可在结束拖动会话后编辑三轴位置、旋转和缩放。

新资产的“资产用途”决定访客行为：

- **仅装饰**：只渲染模型，不创建 hitbox、悬停提示、索引入口或详情页；
- **可交互**：模型可被选中，并根据工作台中的双语标题、介绍、指标和内容卡片生成详情页。

“场景预览”会把工作台收窄到右侧，未被面板覆盖的房间仍可环顾；普通模式下拖动空白区域会旋转视角，摆放会话中拖动选中物体则执行当前的平面、高度或方向操作。数值调整会实时同步。模型缺失或加载失败时会保留一个轻量占位物，避免场景出现不可见但仍可交互的区域。

可交互资产的“内容卡片”使用统一编辑器：

- **纯文字**：标题、眉题、正文和补充信息；
- **图文**：在文字基础上上传一张卡片图片并填写替代文本；
- **链接按钮**：在文字基础上添加最多 4 个安全的 HTTP、HTTPS 或邮件链接按钮；
- 每张卡可选择 **标准（1/3）**、**宽版（1/2）** 或 **整行**，并可上下移动调整页面顺序。

旧版内容卡未填写类型或宽度时会继续按“纯文字 + 标准宽度”显示，不需要手工迁移。
...
## 8. 新增一件程序化内置 3D 资产（高级方式）

普通自有 GLB 请优先使用 Content Studio 的“场景布局”。只有需要程序化几何、专属动画、特殊灯光或深度定制页面时，才需要按本节修改代码。以下以新增“投影机”为例。
